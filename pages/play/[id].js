...
const Play = () => {
  ...
  const [gameRoundColor, setGameRoundColor] = useState([
    'bg-white',
    'bg-white',
    'bg-white',
  ]);
  const [lastRoundResult, setLastRoundResult] = useState('Ready');
  const [thisGameState, setThisGameState] = useState(
    useSelector((state) => state.game.runningGame)
  );
  const [initGameState, setInitGameState] = useState(
    useSelector((state) => state.game.initGame)
  );
  ...
  useEffect(() => {
    if (!thisGameState && id) {
      ...
      if (thisGameState.runningRound.length === Number(game.numberOfRounds)) {
        const gameResult = calculateRounds(thisGameState.runningRound);
  ...
  const handleSuitClick = (e) => {
    if (readyInput === true && gameFinished === false) {
      ...
      const computerInput =
        availableSuit[Math.floor(Math.random() * availableSuit.length)];
      ...
      if (
        Number(newGameState.runningRound.length) === Number(game.numberOfRounds)
      ) {
        const gameResult = calculateRounds(newGameState.runningRound);
        if (gameResult === 'win' || gameResult === 'lose') {
          setLastRoundResult(`You ${gameResult.toUpperCase()} the Game!`);
        } else {
          setLastRoundResult(`Game ${gameResult.toUpperCase()}!`);
    ...
    <>
      ...
      <div className="grid grid-rows-12 grid-cols-3 bg-white border-solid border-[2px] border-[#D7] rounded-lg md:mx-24 md:px-14 xl:mx-28 mt-12 mb-14 xl:px-18">
        ...
        <div className="row-span-1 col-start-2 text-center">
          <p className="lg:text-xl xl:text-2xl font-semibold">
        ...
        <div className="row-start-2 col-start-2 flex justify-center items-center">
          <div
            className={`rounded-full ${gameRoundColor[0]} border-solid border-[1px] border-[#D7D7D7] h-4 w-4 lg:h-5 lg:w-5 xl:h-7 xl:w-7 mt-3 mb-1 mx-2`}
          ></div>
          <div
            className={`rounded-full ${gameRoundColor[1]} border-solid border-[1px] border-[#D7D7D7] h-4 w-4 lg:h-5 lg:w-5 xl:h-7 xl:w-7 mt-3 mb-1 mx-2`}
          ></div>
          <div
            className={`rounded-full ${gameRoundColor[2]} border-solid border-[1px] border-[#D7D7D7] h-4 w-4 lg:h-5 lg:w-5 xl:h-7 xl:w-7 mt-3 mb-1 mx-2`}
          ></div>
        </div>
        ...
        <div className="row-start-4 col-start-1 flex justify-center items-center mt-2">
          <button
            className="focus:bg-slate-200 bg-[length:35px_20px] lg:bg-[length:65px_45px] xl:bg-[length:85px_65px] bg-no-repeat bg-center border-solid border-[4px] lg:border-[8px] border-red-500 rounded-full h-16 w-16 lg:h-28 lg:w-28 xl:h-36 xl:w-36 mt-3 mx-2"
            style={{ backgroundImage: 'url(/img/batu.png)' }}
            disabled={!readyInput}
            onClick={handleSuitClick}
            value="rock"
          />
          <button
            className="focus:bg-slate-200 bg-[length:30px_40px] lg:bg-[length:60px_70px] xl:bg-[length:80px_90px] bg-no-repeat bg-center border-solid border-[4px] lg:border-[8px] border-green-500 rounded-full h-16 w-16 lg:h-28 lg:w-28 xl:h-36 xl:w-36 mt-3 mx-2"
            style={{ backgroundImage: 'url(/img/gunting.png)' }}
            disabled={!readyInput}
            onClick={handleSuitClick}
            value="scissors"
          />
        ...
        <div className="row-start-5 col-start-1 flex justify-center content-start pb-10">
          <button
            className="focus:bg-slate-200 bg-[length:25px_35px] lg:bg-[length:50px_80px] xl:bg-[length:70px_100px] bg-no-repeat bg-center border-solid border-[4px] lg:border-[8px] border-yellow-500 rounded-full h-16 w-16 lg:h-28 lg:w-28 xl:h-36 xl:w-36"
            style={{ backgroundImage: 'url(/img/kertas.png)' }}
            disabled={!readyInput}
            onClick={handleSuitClick}
            value="paper"
          />
        ...
        <div className="row-start-4 col-start-2 flex justify-center items-end">
          <div className="text-2xl lg:text-5xl xl:text-6xl font-black flex items-center text-center mb-8">
            {lastRoundResult}
          </div>
        </div>
        ...
        <div className="row-start-5 col-start-2 flex justify-center items-center">
          <button
            className="cursor-pointer active:bg-slate-200 h-10 w-10 lg:h-16 lg:w-16 xl:h-20 xl:w-20 lg:bg-[length:60px_60px] xl:bg-[length:70px_70px] bg-cover bg-no-repeat bg-center rounded-full"
